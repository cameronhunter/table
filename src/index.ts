...
export default function table(strings: TemplateStringsArray, ...keys: any[]): { [column: string]: any } {
  ...
  return rows.map((values: any[]) => {
    return headers.reduce((state, header, index) => ({ ...state, [header]: values[index] }), {});
  });
}
