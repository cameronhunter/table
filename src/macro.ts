...
function tableMacro({ references, babel: { types: t } }: MacroParams) {
  ...
  function createObject(keys: string[], data: any[]): ObjectExpression {
    return t.objectExpression(
      keys.map((key, index) => t.objectProperty(t.identifier(key), data[index]))
    );
  }

  defaultImport.forEach((referencePath) => {
    const taggedTemplatePath = referencePath.findParent((nodePath) =>
      nodePath.isTaggedTemplateExpression()
    );
    const taggedTemplateNode = taggedTemplatePath?.node;

    if (
      !taggedTemplatePath ||
      !t.isTaggedTemplateExpression(taggedTemplateNode)
    ) {
    ...
    if (!columns) {
      throw new MacroError(
        "Could not determine the column names from the table."
      );
    }

    const rows = getRows(templateLiteral.expressions, columns.length);
    const data = t.arrayExpression(
      rows.map((row) => createObject(columns, row))
    );
